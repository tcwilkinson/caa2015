---
title: "Archaeological Science as Community Enterprise"
author: '[N. Strupler](mailto:nehemie.strupler@etu.unistra.fr)'
date: "02/04/2015 | [CAA 2015 -- KEEP THE REVOLUTION GOING](http://caaconference.org)"
output:
  ioslides_presentation:
    fig_height: 4
    fig_width: 7
    highlight: haddock
    incremental: yes
    logo: ./ASCE-images/Logo-DAI-UDS-t.png
    toc: yes
    transition: 0
    widescreen: yes
subtitle: The Challenge of Reproducibility
bibliography: bibliography.bib
---
<style>

body {  background-color: black; 
}

h2 {  color: #3399ff;    }
h3 {  color: #3399ff;  	}
slide { background-color: #000000;}

ul{ color: #FFF;}
p{ color: #FFF;}
.title-slide hgroup h1 {color: #FFF;}




</style>



# Outline

## Outline {.flexbox .vcenter}


  - What is a scientific community 
  - Similarities with Free Software community
  - Concepts of reuse, modulation and reproducibility
  - Reproducibility in archaeology
  - Conclusion



# The scientific community

## Diversity makes the difference

  - Science is cumulative
  - Differents points of views stabilise knowledge
  - Diversity invigorates problem solving

<div class="centered">
<img src="./ASCE-images/PhDKnowledge.012-Inv.png" alt="PhDKnowledge.012" style="height:380px">
</div>
<footer class="source">Image: [Matt Might](http://matt.might.net/articles/phd-school-in-pictures/)  |  CC BY-NC 2.5 </footer>

## Functions of the scientific community

<div class="columns-2">
 <div class="valign">
  - Inspiration
  - Motivation
  - Scrutiny
</div>

<div class="centered">
<img src="./ASCE-images/MacacaNigraSelfPortrait.jpg" alt="PhDKnowledge.012" style="height:400px">
</div>
<footer class="source">Image: [Self-portrait of a Macaca Nigra](http://commons.wikimedia.org/wiki/File:Macaca_nigra_self-portrait.jpg)   |  CC0 1.0 </footer>
</div>

# Free Software and Science

## Some similarities

<div class="columns-2">
  - Peer review
  - Validation and replication
  - Culture of credit, civility
  - Reputation 
  - Communication
  


<div class="centered">
<img src="./ASCE-images/OpeningScience.png" alt="OpeningScience" style="height:260px">
</div>
<footer class="source">Image: [Bartling et al. 2014, Opening Science (Cover)](http://dx.doi.org/10.1007/978-3-319-00026-8)  |  CC BY-NC 3.0 </footer>  
</div>

### Some dissimilarities

 - How projects are developed, maintained, contested, and how ideas are exchanged
 - Challenge what is taken for granted in science

## Modifiability
  - Centuries of adaptation to 'stable knowledge'
  - New paradigms with constant modulation
  
<div class="centered">
<img src="./ASCE-images/ResearchCulture-Inv.png" alt="ResearchCulture" style="height:380px">
</div>
<footer class="source">Image: [Bartling et al. 2014, Opening Science (fig. 3, 10)](http://dx.doi.org/10.1007/978-3-319-00026-8_1)  |  CC BY-NC 3.0 </footer>  

## Challenge the ‘Power of Knowledge’ | C. Kelty, Two Bits

  - New IT render the knowledge more dynamic (Vinck 2014)
  - New practices: ex. "Push and Publish" workflow (Kansa 2014)
  - Reconcile process and result

<div class="centered">
<img src="./ASCE-images/DynamicPublicationFormats6-Inv.png" alt="DynamicPublicationFormats" style="height:280px">
</div>
<footer class="source">Image: [Heller et al. 2014, Opening Science (fig. 5, 200)](http://dx.doi.org/10.1007/978-3-319-00026-8_13)  |  CC BY-NC 3.0 </footer>




# Reproducibility

## Reproducibility {.build .flexbox .vcenter}

<div class="centered">
<img src="./ASCE-images/KieranHealy--workflow-wide-Inv.png" alt="KieranHealy--workflow" style="height:250px">
</div>
<footer class="source">Image: [Kieran Healy's Homepage](http://kieranhealy.org/resources/) | Kieran Healy </footer>

"We often forget that scientific knowledge is reliable not because scientists are more clever, objective or honest than other people, but because their claims are exposed to criticism and replication." [@Fanelli2013] 



## Pedagogical role {.build}

" In pedagogical terms, Windows is to fish as UNIX is to fishing lessons "

<div class="columns-2">
 - Lions' Commentary on UNIX 6th Edition

<div class="centered">
<img src="./ASCE-images/lions-unix-commentary.jpg" alt="lions-unix-commentary" style="height:250px">
</div>
<footer class="source">Image: [Columbia University Computing History](http://www.columbia.edu/cu/computinghistory/books/) | Frank da Cruz </footer>
</div>

## Pedagogy

" In pedagogical terms, Windows is to fish as UNIX is to fishing lessons "

<div class="columns-2">
> - Lions' Commentary on UNIX 6th Edition
> - An R Companion to Quantifying Archaeology by Stephen Shennan from David L. Carlson





<div class="centered">
<img src="./ASCE-images/Carlson-QuantifiyingArchaeology.png" alt="Carlson-QuantifiyingArchaeology" style="height:250px">
</div>
<footer class="source">Image: [David L. Carlson's Website](http://people.tamu.edu/~dcarlson) </footer>

</div>

## Pedagogy

" In pedagogical terms, Windows is to fish as UNIX is to fishing lessons " 

<div class="columns-2">
> - Lions' Commentary on UNIX 6th Edition
> - An R Companion to Quantifying Archaeology by Stephen Shennan from David L. Carlson
> - Quantitative Archaeology Wiki: [www.isoa.it](http://isoa.it)




<div class="centered">
<img src="./ASCE-images/iosa-DiggingNumbers.png" alt="iosa-DiggingNumbers" style="height:250px">
</div>
<footer class="source">Image: http://wiki.iosa.it </footer>

</div>


## Blurring borders {.build}
<div class="columns-2">

 - (Archaeological) project with R on Hittite seals 

<div class="centered">
<img src="./ASCE-images/RSealProject.png" alt="RSealProject" style="width:430px">
</div>
<footer class="source">Image: [2nd R Meeting, Lyon (France), june 27th-28th, 2013](http://r2013-lyon.sciencesconf.org/) </footer>

</div>



## Blurring borders
<div class="columns-2">

>  - (Archaeological) project with R on Hittite seals 
    - Communication with informal and wider communities 
    - Common language and a common goal through which foreign disciplines are revisited 

<div class="centered">
<img src="./ASCE-images/Rmeeting.png" alt="Rmeeting" style="width:430px">
</div>
<footer class="source">Image: [2nd R Meeting, Lyon (France), june 27th-28th, 2013](http://r2013-lyon.sciencesconf.org/) </footer>

</div>

## Blurring borders
<div class="columns-2">

> - (Archaeological) project with R on Hittite seals 

  - (Statistical) project of Hélène Lombard et Lise Radoszycki

<div class="centered">
<img src="./ASCE-images/RadoszyckiPremiere.png" alt="RadoszyckiPremiere" style="width:430px">
</div>
</div>

## Blurring borders
<div class="columns-2">

> - (Archaeological) project with R on Hittite seals 

> - (Statistical) project of Hélène Lombard and Lise Radoszycki

  - Mixing of competences: two ways ambiguation.
  
  - On-going work on Github: [https://github.com/nehemie/CachetsHittites](https://github.com/nehemie/CachetsHittites)
  
<div class="centered">
<img src="./ASCE-images/CachetsHittitesGithub.png" alt="CachetsHittitesGithub" style="width:430px">
</div>
<footer class="source">Image: [Néhémie Strupler](http://nehemie.github.io/CachetsHittites)  |  CC BY 4.0 </footer>

</div>

# Conclusion

## Key points

 - Relation of FS with archaeology 
 - FS questions transmission and reuse of knowledge
 - Reproducibility makes the research pipeline available for scrutinity
 - Need of further advocacy for FS in archaeology: 
    - Stronger research
    - Pedagogical
    - Innovative


## ...  {.build}
<h3 style="font-size:300%;font-family:Gentium">KEEP THE REVOLUTION GOING</h3>

<!-- <footer class="source"> Thanks to # vegbers </footer> -->

# Thank you 

## Colophon


Presentation written in [Markdown](http://daringfireball.net/projects/markdown/) ([ioslides](http://rmarkdown.rstudio.com/ioslides_presentation_format.html))

Compiled into HTML5 using [RStudio](http://www.rstudio.com/ide/)

Source code hosting: [https://github.com/nehemie/caa2015](https://github.com/nehemie/caa2015)

ORCID: [http://orcid.org/0000-0002-2898-6217](ORCID: http://orcid.org/0000-0002-2898-6217)

<footer class="source">This slide is a modulation of the colophon of [Ben Marwick's](http://orcid.org/0000-0001-7879-4531) [colophon on Reproducibility](http://benmarwick.github.io/UW-eScience-reproducibility-social-sciences/#/39)  |  B. Marwick CC BY 3.0 </footer>


### Licensing:

Presentation: [CC BY 4.0 ](http://creativecommons.org/licenses/by/4.0/)

Source code: [MIT](http://opensource.org/licenses/MIT) 



## Backup slide | Publishing and Pushing
<div class="centered">
<img src="./ASCE-images/KansaWorkflow-Inv.png" alt="KansaWorkflow" style="height:380px">
</div>


<footer class="source">Kansa, E. C.; Kansa, S. W. & Arbuckle, B. [Publishing and Pushing: Mixing Models for Communicating Research Data in Archaeology](http://dx.doi.org/10.2218/ijdc.v9i1.301),  *International Journal of Digital Curation*, 2014, 9.1, 57-70 </footer>

---
nocite: | 
  @Kelty2001, @Kelty2008, @Kansa2014, @Marwick2014, @Vinck2014
...

## References {.smaller}
